...
def euler_maruyama_integrator(
    ...
    for n, t in enumerate(times[:-1]): 
        ...
        x_t[..., n + 1] = (
            ...
            + diffusion_coefficient(t).view(-1, 1, 1) 
              * sqrt_dt
              * z[..., n]
        )
...
class GaussianDiffusionProcess(DiffusionProcess):
    ...
    def loss_function(
        ...
        t = torch.rand(x_0.shape[0], device=x_0.device) * (self.t_end - self.t_0 - self.t_eps) + self.t_eps + self.t_0
        t_broadcast = t.view(-1, *([1]*(x_0.dim()-1)))
        Z = torch.randn_like(x_0)
        x = self.mu_t(x_0, t_broadcast) + self.sigma_t(t).view(-1,1,1) * Z
        loss = ((self.sigma_t(t).view(-1,1,1) * score_model(x,t) + Z)**2).sum(dim=-1).mean()
        return loss
...
def inverse_Tweedie_formula(
    ...
    return inv_drift * (x_t + sigma**2 * score)
