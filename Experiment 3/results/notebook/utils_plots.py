    ...
    if mode == "separated":
        ...
        for row, label in enumerate(labels):
            ...
            for col in range(n_cols):
                ...
                if col < len(idxs):
                    i = idxs[col]
                    ax.plot(axis_x.numpy(), data[i, 0, :].numpy(), lw=1.5, color=color)
        ...
        for col, label in enumerate(labels):
            ...
            idxs = sample_indices[label]
            for i in idxs:
                ax.plot(axis_x.numpy(), data[i, 0, :].numpy(), lw=1.2, alpha=0.8)
            ax.set_title(label)
            ax.grid(True, linestyle="--", alpha=0.6)
            ax.set_xlabel("t")
            if col == 0:
...
def _plot_process_block_vertical(fig, subplot_spec, batch_functions, axis_x, time_idxs, n_samples, time_labels, color):
    ...
    inner = GridSpecFromSubplotSpec(n_steps, n_samples, subplot_spec=subplot_spec, hspace=0.1, wspace=0.05)

    shared_ax = None
    for row, t in enumerate(time_idxs_cpu):
        for col in range(n_samples):
            ...
            y = batch_functions[col, t, 0, :].detach().cpu().numpy()
            ax.plot(axis_x_np, y, lw=1.3, color=color)
            ...
            if col != 0:
                ...
                ax.set_ylabel(time_labels[row], fontsize=8, rotation=0,
                               ha="right", va="center", labelpad=28)
            if row == 0 and n_samples > 1:
                ax.set_title(f"S{col}", fontsize=8)
...
def _plot_two_process_blocks(process_dict, n_samples=3, cell_size=(1.8, 1.8), suptitle="", colors=None):
    ...
    fig_w = n_blocks * n_samples * cell_size[0] * 0.7 + (n_blocks - 1) * 0.8
    fig_h = n_steps_max * cell_size[1] * 0.7 + 0.8

    fig = plt.figure(figsize=(fig_w, fig_h))
    outer = GridSpec(2, n_blocks, height_ratios=[0, 1], hspace=0.03, wspace=0.8, figure=fig)

    for b, label in enumerate(labels):
        batch_fn, axis_x, time_idxs, time_labels = process_dict[label]
        title_ax = fig.add_subplot(outer[0, b]); title_ax.axis("off")
        title_ax.text(0.5, 0.0, label, ha="center", va="bottom", fontsize=13, fontweight="bold")
        _plot_process_block_vertical(fig, outer[1, b], batch_fn, axis_x, time_idxs, n_samples, time_labels, color=model_colors[label])
