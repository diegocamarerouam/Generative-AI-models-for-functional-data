...
def _plot_process_block_horizontal(
    ...
    inner = GridSpecFromSubplotSpec(
        n_samples, n_steps, subplot_spec=subplot_spec, hspace=0.05, wspace=0.10,
    )
...
def _plot_two_process_blocks(process_dict, n_samples=3, cell_size=(1.8, 1.8), suptitle="", colors=None):
    ...
    outer = GridSpec(n_blocks, 2, width_ratios=[0.06, 1], hspace=0.5, wspace=0.02, figure=fig)

    for r, label in enumerate(labels):
        title_ax = fig.add_subplot(outer[r, 0])
        title_ax.axis("off")
        title_ax.text(0.5, 0.5, label, rotation=90, ha="center", va="center",
...
def plot_forward_processes(
    ...
    return _plot_two_process_blocks(process_dict, n_samples=n_samples, cell_size=cell_size,
                                     suptitle=suptitle, colors=colors)
